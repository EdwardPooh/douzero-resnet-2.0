...
class GeneralModelResnet(nn.Module):
    def __init__(self):
        ...
        self.in_planes = 180
        # input 1*54*90
        self.conv1 = nn.Conv1d(90, 180, kernel_size=(3,),
                               stride=(2,), padding=1, bias=False)  # 1*27*180

        self.bn1 = nn.BatchNorm1d(180)
        self.lstm = nn.LSTM(54, 128, batch_first=True)

        self.layer1 = self._make_layer(BasicBlock, 180, 2, stride=2)  # 1*14*180
        self.layer2 = self._make_layer(BasicBlock, 360, 2, stride=2)  # 1*7*360
        self.layer3 = self._make_layer(BasicBlock, 720, 2, stride=2)  # 1*4*720
        self.linear1 = nn.Linear(720 * BasicBlock.expansion * 4 + 19 * 2 + 128, 2048)
